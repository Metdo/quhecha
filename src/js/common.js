...
function setCookie(key, val, iDay) {
	...
	now.setDate(now.getDate() + iDay);//正数就是多少（iDay）天后失效，负数：立即失效
	// document.cookie = key + "=" + val + ";expires" + now + ";path=/";
	document.cookie = key + "=" + val + "; expires=" + now.toUTCString() + ";path=/";
}
//获取cookie
function getCookie(key) {
	var str = document.cookie;//name=Amtion; psw=123456
	var arr = str.split("; ")//[name=Amtion,psw=123456]
	for (var ele of arr) {
		var arr2 = ele.split("=");//[name,Amtion]
		if (key == arr2[0]) {
			return arr2[1];
		}
	}
